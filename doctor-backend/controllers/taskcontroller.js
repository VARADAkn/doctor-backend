...
exports.createTask = async (req, res) => {
  try {
    ...
    await logActivity(req, {
      ...
      entityType: 'TASK',
      entityId: task.id,
      details: { title, assignedTo, patientId, workSpaceId }
    });
...
exports.getTasks = async (req, res) => {
  try {
    ...
    let profileId = req.session.user.profileId;

    // Proactively fetch profileId if missing from session (robustness for stale sessions)
    if (!profileId && (userRole === 'admin' || userRole === 'doctor')) {
    ...
    if (userRole === 'admin') {
      // Admins see tasks they created OR tasks assigned to them (if they are also a doctor)
      const whereConditions = [
        { createdByAdminId: userId }
      ];

      if (profileId) {
        whereConditions.push({ assignedTo: profileId });
      }
      ...
      // Map flag for frontend
      tasks = tasks.map(t => {
        const plain = t.get({ plain: true });
        plain.isMyTask = profileId && String(plain.assignedTo) === String(profileId);
        return plain;
      });
...
exports.updateTask = async (req, res) => {
  try {
    ...
    if (userRole === 'admin') {
      const isCreator = task.createdByAdminId === userId;
      const isAssignee = profileId && task.assignedTo === profileId;
      ...
      if (isCreator) {
        ...
        // Assignees (who are not creators) can only update status
        if (req.body.status) {
          await task.update({ status: req.body.status });
        } else {
          return res.status(400).json({ error: "Assignees can only update task status" });
        }
      }
    } else if (userRole === 'doctor') {
