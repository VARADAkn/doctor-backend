...
def EffNet(input_size, num_classess, pretrained_model, lr_rate, print_trainable_layers = False, print_model_summary = False):
    ...
    model.add(layers.GlobalAveragePooling2D(name='top_gap'))
    model.add(layers.Dense(512, activation='relu', name='dense_512'))
...
callback):
    # Wrap generators for Keras 3 compatibility if needed
    # (Keras 3 stumbles on some custom iterators, treating them as unknown types)
    ...
    # Calculate steps if not provided (though they are passed in)
    if train_step is None:
        train_step = len(train_generator)
    if valid_step is None:
        valid_step = len(validation_generator)

    # Simply wrap the custom iterator in a standard Python generator
    # This often bypasses Keras 3 strict type checking on custom iterator classes
    def python_generator(gen, name="Gen"):
        while True:
            print(f"DEBUG: {name} yielding batch...", flush=True)
            try:
                item = next(gen)
                print(f"DEBUG: {name} batch yielded.", flush=True)
                yield item
            except Exception as e:
                print(f"DEBUG: {name} exception: {e}", flush=True)
                raise e
            
    train_gen_wrapped = python_generator(train_generator, "Train")
    val_gen_wrapped = python_generator(validation_generator, "Val")
...
train_path, image_resize, train_batch_size, validation_batch_size, classes=None):
